...
function storageMode(provider) {
  const fixedEnv = provider === 'hf'
    ? (process.env.HF_TOKEN || process.env.HUGGINGFACE_API_KEY)
    : process.env.GROQ_API_KEY;
  if (fixedEnv) return 'env';
...
// Mesmo esquema de armazenamento da Groq, só que numa chave separada
// (arquivo local: campo "hfApiKey"; KV: "hf_api_key"), pra não misturar as duas.
// HF_TOKEN (ou HUGGINGFACE_API_KEY) no ambiente sempre vence, igual a GROQ_API_KEY.
async function getHfKey() {
  const envKey = process.env.HF_TOKEN || process.env.HUGGINGFACE_API_KEY;
  if (envKey) return envKey.trim();
  if (hasKvStore) {
    try { const v = await kvGet('hf_api_key'); if (v) return v; } catch (e) {}
  }
  const local = readLocal();
  return (local && local.hfApiKey) || '';
}

async function setHfKey(key) {
  if (process.env.HF_TOKEN || process.env.HUGGINGFACE_API_KEY) {
    return { ok: false, reason: 'HF_TOKEN está fixo por variável de ambiente neste servidor — remova-a das variáveis de ambiente pra poder trocar por aqui.' };
  }
  if (hasKvStore) {
    const ok = await kvSet('hf_api_key', key);
    return { ok, reason: ok ? null : 'Falha ao gravar no KV.' };
  }
  const cur = readLocal() || {};
  cur.hfApiKey = key;
...
module.exports = { getGroqKey, setGroqKey, getHfKey, setHfKey, storageMode, hasKvStore, isVercel };
